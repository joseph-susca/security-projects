...
std::vector<STIGFinding> load_from_csv(std::string filename) {
  ...
  while (std::getline(infile, line)) {
    ...
    temp.id = line.substr(0, pos);
    line = line.substr(pos + 1);
  
    pos = line.find(",");
    temp.category = line.substr(0, pos);
    line = line.substr(pos + 1);

    pos = line.find(",");
    temp.description = line.substr(0, pos);
    line = line.substr(pos + 1);
...
int main() {
  ...
  std::cout << "Welcome to STIG Analyzer v2.0\n" << "1. Load findings from CSV file\n" << "2. Enter findings manually\n";
  std::cin >> menu_choice;
  ...
  if (menu_choice == 1) {
    ...
    std::cin >> filename;
    findings = load_from_csv(filename);
  }
  else {
